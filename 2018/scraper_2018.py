...
def get_weeks_games(week):
    print(week)
    new_games = []
    teams = pandas.read_csv("2018_audl_teams.csv")
    ...
    for game_table in game_tables:
        ...
        dt = datetime.datetime.strptime(date_string + " 2018","%m/%d %Y").date()
        ...
        if not ((week == 'week-7') and (home_team == "PHI")):
...
def run():
    ...
    games.to_csv("2018_audl_games.csv")
