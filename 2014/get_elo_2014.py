...
def fill(teams,games):
    ...
    games["year_id"] = pd.Series(["2014"]*(num_games+1))
...
if __name__ == "__main__":
    teams = pd.read_csv("2014_audl_teams.csv")
    teams.set_index('abbr',inplace=True)
    games = pd.read_csv("2014_audl_games.csv",index_col=0)
    ...
    games.to_csv("2014_audl_elo.csv")
